// finish-book.js — Capacitor/Android version
...
function displayBronzeBadge() {
    ...
    if (completedStory) {
        completionMessage.innerHTML = `
            ...
            You've earned a <strong style="color:#CD7F32;">BRONZE BADGE</strong>
            for completing <strong>"${completedStory.title}"</strong>!<br><br>
        `;
    }
}
...
// CHANGED: was ipcRenderer.invoke(...) for each step
async function saveStoryCompletion() {
    try {
        const currentUser = JSON.parse(localStorage.getItem('currentUser'));
        if (currentUser && completedStory) {
            for (let i = 1; i <= completedStory.totalSegments; i++) {
                await db.saveProgress(currentUser.id, completedStory.id, i);
            }
            await db.awardBadge(currentUser.id, completedStory.id, 'bronze', 'story-completion');
