...
// ── SETUP FORMS ───────────────────────────────────────────────────────────────
// Wait for DB to be ready before wiring up forms, so db.login/register works
...
// If DB is already ready (app-init.js fired before us), set up forms now.
// Otherwise wait for the db:ready event.
if (window.__dbReady) {
    setupForms();
} else {
    document.addEventListener('db:ready', setupForms, { once: true });
}